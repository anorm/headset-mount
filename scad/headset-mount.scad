...
table_thickness = 17;  // [5:0.1:50]
thick = 4;             // [3:0.5:7]
drop = 40;             // [25:1:80]
headset_width = 30;    // [15:1:60]
clamp_length=25;       // [20:1:50]
clamp_pinch_angle=1;   // [0:0.1:2]
R=3;                   // [0:0.1:5.5]
width=40;              // [20:1:60]
head_rise = 4;         // [0:0.5:20]
extra_lip=3;           // [0:0.5:10]
lip=head_rise+extra_lip;

pinch = clamp_length * tan(clamp_pinch_angle);
echo("Pinch", pinch);

/*
    Shape seen from the side. Point K is [0,0]
...
*/
P = [
    /* A */ [-thick,              table_thickness+thick+thick/2, R],
    /* B */ [-thick,              -thick,                        R],
    /* C */ [headset_width,       -thick,                        2*R-thick/2],
    /* D */ [headset_width,       -thick-drop,                   2*R-thick/2],
    /* E */ [0,                   -thick-drop,                   2*R-thick/2],
    /* F */ [0,                   -thick-drop+lip,               2*R-thick/2],
    /* G */ [-thick,              -thick-drop+lip,               2*R-thick/2],
    /* H */ [-thick,              -thick-drop-thick,             2*R+thick/2],
    /* I */ [headset_width+thick, -thick-drop-thick,             2*R+thick/2],
    /* J */ [headset_width+thick, 0,                             2*R+thick/2],
    /* K */ [0,                   0,                             0],
    /* L */ [0,                   table_thickness,               0],
    /* M */ [clamp_length,        table_thickness-pinch,         R],
    /* N */ [clamp_length,        table_thickness+thick-pinch,   R]
    ];

// Working from right side (YZ as XY)
rotate([90,0,90]) {
    /*
    translate([0,0,width/2])
    color("red")
    stroke(submatrix(P,[0:len(P)-1],[0:1]), closed=true);
    */

    roundedPoints = polyRound(P, 36);
    linear_extrude(width,center=true)
        polygon(roundedPoints);
    ...
    if (head_rise > 0) {
        ...
        xrot(90)
        yrot(90)
        translate([-thick-drop,0,0])
        linear_extrude(headset_width)polygon(path);
    }
}
...
/*****************************************************************************
  ...
  The following libraries are copied here since makerworld does not support
